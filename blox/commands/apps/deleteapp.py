...
import subprocess
import click
from ..config import BASE_PATH, APPS_TXT_PATH, CUSTOM_APPS_PATH, NEXTJS_PATH
import platform
...
def deleteapp(app_name):
    ...
    # Delete the custom app folder and its content
    docspath = os.path.join(NEXTJS_PATH, "src", "pages", app_name)
    ...
    django_process = subprocess.Popen(
        [python_command, "manage.py", "makemigrations"], cwd=BASE_PATH
    )
    django_process.communicate()

    django_process = subprocess.Popen(
        [python_command, "manage.py", "migrate"], cwd=BASE_PATH
    )
    django_process.communicate()
