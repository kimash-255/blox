...
const DocumentForm = forwardRef(({ config, initialData, onSubmit }, ref) => {
  ...
  const renderFields = (fields) => {
    return fields.map((field) => {
      ...
      const fieldConfig = {
        ...
        select: (
          <select {...commonProps} className={`${commonProps.className} p-2.5`}>
            <option key="" value="">
              Select {field.name}
            </option>
        ...
        <div key={field.id1} className="col-span-6 sm:col-span-3">
          ...
          >
            <div className="flex items-center space-x-2">
              <FontAwesomeIcon icon={icon} className="h-4 w-4 text-gray-500" />
  ...
  return (
    <form ref={formRef} onSubmit={handleSubmit} className="py-4 mx-4 space-y-4">
      {renderTabs()}
